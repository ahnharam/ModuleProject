    ...
    internal class DeviceStateViewModel : BindableBase
    {
            ...
            else
            {
                foreach (var device in Devices)
                {
                    ...
                    if (check == false)
                    {
                        ...
                        deviceGroupModel.MasterName = device.MasterName;
        ...
        private void GatewayConnect(object obj)
        {
            ...
            Task.Run(async () =>
            {
                while (GateWaySendData)
                {
                    ...
                    LogMessage.Instance.DebugLog("Queue Count : " + _gateWayConnect._PoleLogData.Count);
                    ...
                    while (_gateWayConnect._PoleLogData.Count != 0)
                    {
                        ...
                        if (data != null)
                        {
                            DatabaseConnect.Instance.InsertPoleUI(poleLogModel.MId, Convert.ToInt32(poleLogModel.Id), data);
                        }
                        else
                        {
                            // 장애처리
                            DatabaseConnect.Instance.InsertPoleUI(poleLogModel.MId, Convert.ToInt32(poleLogModel.Id), data, 0);
                            DatabaseConnect.Instance.InsertWirelessPlusPoleUI(poleLogModel.MId, Convert.ToInt32(poleLogModel.Id), false);
                        }
                    }
                }
            });
        ...
        private async void GatewaySendData(object obj)
        {
            await Task.Run(async () =>
            {
                ...
                if (DeviceGroupModels == null)
                {
                    LogMessage.Instance.DataLogWrite("등록된 Master가 존재하지 않습니다.");
                    return;
                }
                ...
                if (deviceGroupModel == null)
                {
                    ...
                    LogMessage.Instance.LogWrite("SelectDeviceSendData : " + SelectDeviceSendData);
                _gateWayConnect.DeviceLoadDataOne(deviceGroupModel.Devices, SelectDeviceSendData, SelectDevice);
