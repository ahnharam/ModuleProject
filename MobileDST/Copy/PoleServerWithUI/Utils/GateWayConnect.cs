...
{
    class GateWayConnect : BindableBase
    {
                    ...
                    while (_ConnectedGateway)
                    {
                        ...
                        foreach(var device in deviceModels)
                        {
                            if (device.Ip.Equals(((IPEndPoint)client.Client.RemoteEndPoint).Address.ToString()))
                            {
                                ...
                                DispatcherService.Invoke((System.Action)(() =>
                                {
                                    LogMessage.Instance.LogWrite(String.Format("클라이언트 IP : {0}, Connection : {1}", data.Ip, data.Connected));
        ...
        public void DataLoad(string sendData, DeviceModel device, TcpClient client)
        {
            ...
            try
            {
                ...
                byte[] msg = ConvertHexStringToByte(sendData.Replace(" ", ""));
                ...
                byte[] completeMsg = ConvertHexStringToByte(complete.Replace(" ", ""));
                //completeMsg = { 0x1B,};

                stream.Write(completeMsg, 0, completeMsg.Length);
                DispatcherService.Invoke((System.Action)(() =>
                {
                    //LogMessage.Instance.Log.Add(String.Format("CRC : {0}", crc));
            ...
            catch (Exception e) 
            {
                DispatcherService.Invoke((System.Action)(() =>
                {
                    ...
                    if (check)
                    {
                        DatabaseConnect.Instance.InsertWirelessPlusPoleUI(Convert.ToInt32(device.Sid), false);
                    }
        ...
        public PoleLogModel DataSave(string responseData, DeviceModel deviceModel)
        {
            ...
            if (crcCheck)
            {
                ...
                poleLogModel.Name = deviceModel.Name;
                poleLogModel.Screen = deviceModel.Screen;
                poleLogModel.Type = deviceModel.Type;
                ...
                poleLogModel.Temp = Convert.ToInt32(datas[22] + datas[21], 16).ToString();
                poleLogModel.Outtemp = Convert.ToInt32(datas[24] + datas[23], 16).ToString();
